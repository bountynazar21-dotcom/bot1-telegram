...
async def cancel_all(message: Message, state: FSMContext):
    await state.clear()
    await message.answer("🔁 Скасовано. Введіть /start, щоб почати знову.", reply_markup=cancel_keyboard)
...
async def sender_group(message: Message, state: FSMContext):
    ...
    await message.answer("📍 Оберіть точку відправника:", reply_markup=point_keyboard(group))
...
async def sender_point(message: Message, state: FSMContext):
    ...
    await message.answer("📦 Оберіть підгрупу отримувача:", reply_markup=group_keyboard())
...
async def receiver_group(message: Message, state: FSMContext):
    ...
    await message.answer("📥 Оберіть точку отримувача:", reply_markup=point_keyboard(group))
...
async def receiver_point(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Ви обрали точки:\n📤 Відправник: {sender['name']} ({sender['id']})\n"
